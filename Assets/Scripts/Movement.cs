...
public class Movement : MonoBehaviour 
{
	...
	public void Move(Vector3 desired_velocity)
	{
		...
		Vector2 new_velocity = GetComponent<Rigidbody2D>().velocity + (Vector2)desired_velocity.normalized * acceleration * Time.deltaTime;
        if (new_velocity.x >= desired_velocity.x || new_velocity.y >= desired_velocity.y)
