...
public class AStarTargeter : Targeter {
	...
	private ArrayList GetAStarPath() {
		...
		while (frontier.Count != 0) {
			...
			if (currentNode.Equals(goalNode)) {
				...
				path = getSmoothedPath(path, layerMask);
	...
	private ArrayList getSmoothedPath(ArrayList unsmoothed_path, int layerMask)
    {
		ArrayList smoothed_path = new ArrayList();

		// TODO: Add code here to remove unnecessary nodes from smoothed_path
		Grid grid = (Grid)(grids[gameObject.tag]);
		Vector2 startPoint = (Vector2)unsmoothed_path[0];
		smoothed_path.Add(startPoint);

		int samples = 4;
		for (int i = 1; i < unsmoothed_path.Count; i++) {
			Vector2 endPoint = (Vector2)unsmoothed_path[i];
			if (!Physics2D.Raycast(grid.GetWorldPosition(startPoint), grid.GetWorldPosition(endPoint), float.MaxValue, layerMask)) {
				startPoint = (Vector2)unsmoothed_path[i - 1];
				smoothed_path.Add(startPoint);
            } else {
				for (int s = 1; i <= samples; i++)
				{
					Vector2 offset = grid.GetWorldPosition(startPoint) - (Vector2)GameObject.Find("Player/Sample" + s).transform.position;
					Vector2 tempStartPoint = startPoint + offset;
					Vector2 tempEndPoint = endPoint + offset;

					if (!Physics2D.Raycast(grid.GetWorldPosition(tempStartPoint), grid.GetWorldPosition(tempEndPoint), float.MaxValue, layerMask)) {
						startPoint = (Vector2)unsmoothed_path[i - 1];
						smoothed_path.Add(startPoint);
						break;
					}
		...
		if (!smoothed_path.Contains(unsmoothed_path[unsmoothed_path.Count - 1]))
        {
			smoothed_path.Add(unsmoothed_path[unsmoothed_path.Count - 1]);
